 ...
 * 
 * Provides AES-256-GCM encryption for API keys without requiring Google Cloud KMS.
 * This is suitable for development and testing environments.
 * 
 * For production, use the full envelope encryption with Google Cloud KMS.
 */
...
export class SimpleEncryptionService {
  ...
  /**
   * Encrypt API key using AES-256-GCM with salt and IV
   */
  async encryptApiKey(apiKey: string, customerContext: string): Promise<SimpleEncryptionResult> {
    try {
      ...
      console.error('Simple encryption failed:', {
  ...
  async decryptApiKey(
    ...
    try {
      ...
      console.error('Simple decryption failed:', {
        error: error instanceof Error ? error.message : 'Unknown error',
        timestamp: new Date().toISOString()
      });
      return { decryptedData: '', isValid: false };
